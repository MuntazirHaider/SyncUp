...
export default async function handler(
  ...
  try {
    ...
    if (!messageId)
      return res.status(404).json({ message: "Member ID missing" });
    if (!content && req.method === "PATCH") return res.status(404).json({ message: "Content missing" });
    ...
    if (req.method === "PATCH") {
      if (!isMessageOwner) {
        return res
          .status(401)
          .json({ message: "Unauthorized for this action" });
      }
